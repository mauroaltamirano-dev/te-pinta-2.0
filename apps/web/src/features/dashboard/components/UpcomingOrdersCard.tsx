...
const ActionButton = ({ children, onClick }: { children: ReactNode; onClick?: () => void }) => (
  <button
    className="min-h-9 rounded-xl border border-border bg-white px-3 text-xs font-black text-sidebar transition-colors hover:border-primary/40 hover:bg-primary/5 hover:text-primary"
    onClick={onClick}
    type="button"
  >
    {children}
...
export const UpcomingOrdersCard = ({ orders }: { orders: DashboardOrderCard[] }) => (
        ...
        {orders.slice(0, 5).map((order) => (
          ...
          >
            <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm font-bold text-muted-foreground">
                  {getDashboardOrderCode(order.id)} · {order.deliveryLabel}
                </p>
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <ActionButton>Marcar producido</ActionButton>
              <ActionButton>Marcar pagado</ActionButton>
              <ActionButton>Confirmar entrega</ActionButton>
            </div>
          </article>
        ))}
      </div>
