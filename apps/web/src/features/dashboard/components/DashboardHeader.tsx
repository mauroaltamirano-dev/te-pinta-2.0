...
export const DashboardHeader = ({ date }: { date: string }) => (
  <header className="relative overflow-hidden rounded-[1.35rem] border border-white/70 bg-sidebar p-4 text-white shadow-card lg:rounded-[1.75rem] lg:p-5">
    ...
    <div className="relative flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
      <div>
        <p className="hidden items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-crema-maiz ring-1 ring-white/10 lg:inline-flex">
          <Gauge className="size-3.5" aria-hidden /> Centro operativo
        </p>
        <h1 className="font-display text-2xl font-black tracking-tight lg:mt-3 lg:text-4xl">
          Dashboard general
        </h1>
        <p className="mt-1 hidden max-w-2xl text-sm font-semibold text-sidebar-muted lg:block">
          Resumen operativo y financiero del emprendimiento.
        </p>
        <div className="mt-3 hidden flex-wrap gap-2 text-sm font-black text-white lg:flex">
      ...
      <div className="grid grid-cols-2 gap-2 lg:flex lg:items-center">
        <Link
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-3 text-xs font-black text-white shadow-primary-glow transition-colors hover:bg-primary/90 lg:px-5 lg:text-sm"
          to="/orders"
        >
          <Plus className="size-4" aria-hidden /> Nuevo pedido
        </Link>
        <Link
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-white/20 bg-white/10 px-3 text-xs font-black text-white transition-colors hover:bg-white/15 lg:px-5 lg:text-sm"
